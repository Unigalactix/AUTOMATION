...
function generateCopilotPrompt({ issueKey, summary, description, repoConfig, repoName, defaultBranch, language }) {
    ...
    return `@copilot /fix This issue first **${issueKey}: ${summary}**

${description || ''}

and then Read the whole repository first and then Please generate a CI / CD pipeline file based on this repo using the format below(ignore if already exists and working):
    \`\`\`yaml
...
async function createPullRequestForWorkflow({ repoName, filePath, content, language, issueKey, deployTarget, defaultBranch, repoConfig, ticketData }) {
    try {
        ...
        let promptBody = '';
        if (ticketData) {
            console.log('Generating Copilot Prompt for PR Body...');
            promptBody = generateCopilotPrompt({
                issueKey,
        ...
        // 8. If we skipped adding a new workflow, attempt to trigger existing workflow on feature branch
